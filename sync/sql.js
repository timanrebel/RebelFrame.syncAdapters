...
function Sync(method, model, opts) {
	var db, sql, table = model.config.adapter.collection_name,
	...
	switch (method) {
		...
		case "read":
			...
			opts.query ? sql = opts.query : model.id && (sql += " WHERE " + model.idAttribute + " = " + model.id);
			db = Ti.Database.open(dbName);
			var rs;
...
function Migrate(Model) {
	...
	if (migrations.length)
		for (var i = 0; migrations.length > i; i++) {
